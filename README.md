...
└──────┬─────────────────────┬─────────────────┘
       ...
       │ 실시간 센서 구독     │ Factory API
┌──────▼─────────────────────▼─────────────────┐
│  로컬 PC (Docker)                             │
│  RabbitMQ (MQTT/STOMP/Web STOMP)             │
│  PostgreSQL + Redis + InfluxDB               │
│  Spring Boot Factory API                     │
└──────┬───────────────────────────────────────┘
       │ MQTT (1883)
┌──────▼───────────────────────────────────────┐
│  RPi                                          │
│  ugv_bridge (MQTT publisher + REST API:8081) │
│  ROS 2 노드 (모터/센서/LiDAR/Nav2)           │
└──────────────────────────────────────────────┘
       ↑ REST (8081)
       │ 로봇팔/그리퍼/네비게이션/맵
       └── 웹 브라우저에서 직접 호출
...
├── .env                         # VITE_ROBOT_HOST, VITE_BRIDGE_HOST (gitignored)
...
VITE_BRIDGE_HOST=192.168.0.71   # RPi IP
VITE_API_PORT=8081
...
| `VITE_BRIDGE_HOST` | ugv_bridge REST API 호스트 (RPi) | (미설정 시 STOMP 호스트 사용) |
| `VITE_API_PORT` | ugv_bridge REST API 포트 | `8081` |
