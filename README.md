...
└──────┬─────────────────────┬─────────────────┘
       ...
       │ 실시간 센서 구독     │ Factory API + 로봇 프록시
┌──────▼─────────────────────▼─────────────────┐
│  로컬 PC (Docker)                             │
│  RabbitMQ (MQTT/STOMP/Web STOMP)             │
│  PostgreSQL + Redis + InfluxDB               │
│  Spring Boot Factory API (로봇 프록시 포함)   │
└──────┬──────────────────────┬────────────────┘
       │ MQTT (1883)          │ REST 프록시
┌──────▼──────────┐    ┌──────▼──────────┐
│  RPi (ugv01)     │    │  RPi (ugv02)     │
...
브라우저는 **Factory API를 리버스 프록시로 사용**하여 각 로봇의 bridge에 접근합니다:
- `POST /api/robots/ugv01/arm` → Factory API → `http://192.168.0.71:8081/api/ugv01/arm`
- `POST /api/robots/ugv02/navigate` → Factory API → `http://192.168.0.72:8081/api/ugv02/navigate`

로봇 등록/관리는 Factory API의 CRUD 엔드포인트로 수행합니다:
- `GET /api/robots` — 등록된 로봇 목록
- `POST /api/robots` — 새 로봇 등록 (`{ "id": "ugv02", "name": "UGV-02", "bridgeUrl": "http://192.168.0.72:8081" }`)
...
| HTTP 클라이언트 | axios |
| 명령/조회 | REST → Factory API 프록시 → RPi bridge (FastAPI) |
| 메시지 브로커 | RabbitMQ (로컬 PC Docker, MQTT + STOMP + Web STOMP) |
| 백엔드 브릿지 | ugv_bridge (RPi, 별도 [ugv_ws](https://github.com/fhekwn549/ugv_ws) 레포) |
| 공장 백엔드 | Spring Boot + PostgreSQL + Redis + InfluxDB (ugv_factory/) |
| 로봇 관리 | Factory API: 로봇 CRUD + 리버스 프록시 + 헬스체크 |
...
### REST API (로봇 제어 — Factory API 프록시 경유)

| 메서드 | 엔드포인트 | 용도 |
|--------|-----------|------|
| POST | `/api/robots/{id}/navigate` | Nav2 목표 전송 (x, y, theta) |
| POST | `/api/robots/{id}/cancel` | 네비게이션 취소 |
| POST | `/api/robots/{id}/initial_pose` | AMCL 초기 위치 설정 |
| POST | `/api/robots/{id}/arm` | 로봇팔 관절 명령 |
| POST | `/api/robots/{id}/gripper` | 그리퍼 명령 |
| GET | `/api/robots/{id}/map` | 맵 PNG (base64) + 메타데이터 |
| GET | `/api/robots/{id}/logs?type=command` | 명령 로그 조회 |
| GET | `/api/robots/{id}/logs?type=event` | 이벤트 로그 조회 |
| GET | `/api/robots/{id}/logs/navigation` | 네비게이션 로그 조회 |
...
|--------|-----------|------|
| GET | `/api/robots` | 등록된 로봇 목록 |
| GET | `/api/robots/{id}` | 로봇 상세 (bridgeUrl, status) |
| POST | `/api/robots` | 로봇 등록 |
| PATCH | `/api/robots/{id}` | 로봇 정보 수정 |
...
| 변수 | 용도 | 기본값 |
|------|------|--------|
| `VITE_ROBOT_HOST` | STOMP 브로커 + Factory API 호스트 (로컬 Docker) | `localhost` |
| `VITE_FACTORY_API_PORT` | Factory API 포트 | `8082` |

> 로봇별 RPi IP는 Factory API의 Robot DB에서 관리합니다. `.env`에 개별 RPi 주소를 설정할 필요가 없습니다.
...
```powershell
# PowerShell (관리자 권한)
New-NetFirewallRule -DisplayName "UGV Docker Services" -Direction Inbound -LocalPort 1883,5672,15672,15674,61613,8082 -Protocol TCP -Action Allow
```
